...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/06 19:42:51 by arakhurs          #+#    #+#             */
/*   Updated: 2023/11/04 18:45:38 by arakhurs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
		_env = copy._env;
		_ch_env = copy._ch_env;
		_argv = copy._argv;
		_cgi_path = copy._cgi_path;
		_cgi_pid = copy._cgi_pid;
		_exit_status = copy._exit_status;
}
...
void    CgiHandler::setCgiPid(pid_t cgi_pid){_cgi_pid = cgi_pid;}
void    CgiHandler::setCgiPath(const std::string &cgi_path){_cgi_path = cgi_path;}
// std::string setCookie(const std::string& str);
const std::map<std::string, std::string> &CgiHandler::getEnv() const{return (_env);}
const pid_t &CgiHandler::getCgiPid() const{return (_cgi_pid);}
const std::string &CgiHandler::getCgiPath() const{return (_cgi_path);}
...
void    CgiHandler::initEnv(HttpRequest& req, const std::vector<Location>::iterator it_loc)
{
    ...
    _env["SCRIPT_FILENAME"] = ((poz < 0 || (size_t)(poz + 8) > _cgi_path.size()) ? "" : _cgi_path.substr(poz + 8, _cgi_path.size())); // check dif cases after put right parametr from the response
