...
if 'HTTP_COOKIE' in os.environ: 
    ...
    if "SID" in cookies:
        session_id = cookies["SID"].value
        session_file_path = 'cgi-bin/sessions/session_' + session_id

        if os.path.exists(session_file_path):
            print("Your Session ID is", session_id, file=sys.stderr)
            ...
            cookie["SID"] = ""
            cookie["SID"]["expires"] = 0  # Set expiration to a past date
            print("Content-type: text/html\r\n")
            print("<html>")
            print("<head>")
            print("<title>Session Expired</title>")
            print("</head>")
            print("<body>")
            print("<h1>Session Expired</h1>")
            print("<p>Your session has expired. Please log in again.</p>")
            print("<a href=\"/login.html\"> Click here to go to the login page </a>")
            print("</body>")
            print("</html>")
    else:
        handleLogin()
